...
export const api = {
  ...
  tankHistory: (tank_id = "T1", limit = 100, since?: string) =>
    http<{ items: TankHistoryRow[] }>(`/tank/history?tank_id=${encodeURIComponent(tank_id)}&limit=${limit}${since ? `&since=${encodeURIComponent(since)}` : ""}`),
  ...
  dashboardSummary: (zone_id = "Z1", tank_id = "T1", alerts_limit = 5, events_limit = 5) =>
    http<DashboardSummary>(`/dashboard/summary?zone_id=${encodeURIComponent(zone_id)}&tank_id=${encodeURIComponent(tank_id)}&alerts_limit=${alerts_limit}&events_limit=${events_limit}`),
